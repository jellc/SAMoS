...
class Plane:
  ...
  def write(self,outfile):
    ...
    out.write('# id  radius  x   y   z   vx   vy   vz   nx   ny   nz   nvx   nvy   nvz   area    type   boundary\n')
    for p in self.particles:
      x, y, z = p.r
      vx, vy, vz = p.v
      nx, ny, nz = p.n
      out.write('%d  %d  %f %f  %f  %f  %f  %f  %f  %f  %f  %f\n' % (p.idx,p.R,x,y,z,vx,vy,vz,nx,ny,nz,p.tp))
    out.close()
...
class Circle:
  ...
  def write(self,outfile):
    ...
    out.write('# Generated on : %s\n' % str(gentime))
    area = self.R * self.R * pi / (self.N + self.Nbound)
    out.write('keys:  id  radius  x   y   z   vx   vy   vz   nx   ny   nz   nvx   nvy   nvz   area    type   boundary\n')
    # out.write('# id  radius  x   y   z   vx   vy   vz   nx   ny   nz   nvx   nvy   nvz   area    type   boundary\n')
    ...
    for p in self.particles:
      ...
      vx, vy, vz = p.v
      nx, ny, nz = p.n
      out.write('%d %f %f %f  %f  %f  %f  %f  %f  %f  %f  0.0  0.0  1.0 %f  2  0\n' % (p.idx,p.R,x,y,z,vx,vy,vz,nx,ny,nz,area))
      
    for p in self.boundparticles:
      x, y, z = p.r
      vx, vy, vz = p.v
      nx, ny, nz = p.n
      out.write('%d %f %f %f  %f  %f  %f  %f  %f  %f  %f  0.0  0.0  1.0 %f  1  1\n' % (p.idx,p.R,x,y,z,vx,vy,vz,nx,ny,nz,area))

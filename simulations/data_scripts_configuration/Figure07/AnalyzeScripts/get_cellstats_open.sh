...
## This script computes the various statistical quantities for each run using AnalyzeCellConfiguration.py, available in the utils folder of the SAMoS package.
## This python script calls on CellConfiguration to perform the actual data read and analysis for each snapshot saved as text file (one for positions and one for boundaries).
## AnalyzeCellConfiguration.py then computes a number of quantities (e.g. p_0 distributions and borderlengths) and saves them as a python dictionary which is then stored as a python pickle file, like this:
...
# Location where the pickle files of each run are stored
		...
		do
			...
			do
				...
				do
				      ...
				      echo confdir
				      echo python ${topdir_python}AnalyzeCellConfiguration.py -i cell_000 -f faces_000 -c ${parfile} -d ${confdir} -o ${topdir_pickle} --prefix Open_v${v}_nu${nu}_gam${gamma}_per${pm}_line${line}
				      python ${topdir_python}AnalyzeCellConfiguration.py -i cell_000 -f faces_000 -c ${parfile} -d ${confdir} -o ${topdir_pickle} --prefix Open_v${v}_nu${nu}_gam${gamma}_per${pm}_line${line}
				done
			done
		done
	done
